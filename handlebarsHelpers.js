...
Handlebars.registerPartial('song', '<span class="song" ng-click="playSong($event)" ng-right-click="showSongContextMenu($event)" data-id="{{song_id}}" data-artist="{{artist_id}}" data-album="{{album}}"><i class="fa fa-ellipsis-v contextClick" ng-click="showContextMenu($event)" ng-hide="!isMobile"></i> {{name}}&nbsp;-&nbsp;{{title}}</span>');
